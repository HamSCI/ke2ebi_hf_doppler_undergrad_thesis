...
if __name__ == '__main__':
    ...
    fig   = plt.figure(figsize=(36 * ncols, nrows * 10))         # increase overall figure size
    ...
    # ── Colorbar ──────────────────────────────────────────────────────────────
    # Find the first mappable (pcolormesh/imshow) across all panels
    mappable = None
    for row_axs in axs:
        for ax in row_axs:
            if ax.collections:
    ...
    if mappable is not None:
        cbar_ax = fig.add_axes([0.92, 0.05, 0.02, 0.90])  # [left, bottom, width, height]
        cbar = fig.colorbar(mappable, cax=cbar_ax)
        cbar.set_label('Relative Strength (dB)', fontsize=40, fontweight='normal', labelpad=20)
    ...
    fig.tight_layout(w_pad=4, h_pad=6, rect=[0, 0, 0.91, 1])  # rect reserves space for colorbar
